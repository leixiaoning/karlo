...
class StableDiffusionImageVariationPipeline(DiffusionPipeline):
    ...
    def mixlxn(self, image_embeddings):
        _emb, embimg = image_embeddings.chunk(2)
        emb0, emb1 = embimg.chunk(2)
        mixemb = []
        for interp_step in torch.tensor([0.4, 0.5]): #torch.linspace(0, 1, 5):
            temp_image_embeddings = self.slerp(interp_step, emb0[0][0], emb1[0][0]).unsqueeze(0)
            mixemb.append(temp_image_embeddings)
        
        mixemb = torch.cat(mixemb).unsqueeze(1) #.to(device)
        mixemb = torch.cat([_emb, mixemb])
        return mixemb
    ...
    def __call__(
        ...
        if mixemb:
            image_embeddings = self.mixlxn(image_embeddings)
