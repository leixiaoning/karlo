...
class sdsr():
    def __init__(self, scheduler, cpu=False, xfm=False):
        self.pipe_g = self.make_pipeline_generator(cpu=cpu)
        self.pipe_up = self.make_pipeline_upscaler(scheduler=scheduler, cpu=cpu, xfm=xfm)

    #@st.cache(allow_output_mutation=True, show_spinner=False, max_entries=1)
    def make_pipeline_generator(self, cpu=False):
        """Create Karlo pipeline"""
        pipe = UnCLIPPipeline.from_pretrained(
            "stable-karlo/kakaobrain/karlo-v1-alpha", torch_dtype=torch.float16
        )
        pipe = pipe.to("cuda")
        if cpu:
    ...
    #@st.cache(allow_output_mutation=True, show_spinner=False, max_entries=1)
    def make_pipeline_upscaler(self, scheduler, cpu=False, xfm=False):
        """Create Stable-Diffusion upscaler pipeline with scpecified scheduler"""
        if scheduler == "Euler":
            scheduler = EulerDiscreteScheduler.from_pretrained(
                "stable-karlo/stabilityai/stable-diffusion-x4-upscaler", subfolder="scheduler"
            )
        elif scheduler == "LMS":
        ...
        pipe = StableDiffusionUpscalePipeline.from_pretrained(
            "stable-karlo/stabilityai/stable-diffusion-x4-upscaler",
            scheduler=scheduler,
    ...
    def upscale(self, cpu, xfm, downscale, scheduler, prompt, neg_prompt, images, n_steps, cfg):
